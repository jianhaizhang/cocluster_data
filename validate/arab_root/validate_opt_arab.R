...
# Data paths.
dat.in <- '~/bigdata/single_cell/data'
res <- '~/bigdata/single_cell/result'
...
# Bulk tissues of Arabdopsis root.
blk.arab.rt <- bulk_dat(rds=file.path(res, 'blk_data.rds'), cnt.path='~/bigdata/single_cell/COPILOT/supp_data/published_datasets/RNASeq_counts.csv', meta.path='~/bigdata/single_cell/COPILOT/supp_data/ici_metadata.csv', marker.blk.path='~/bigdata/single_cell/cell_type_marker_bulk.txt')
blk.arab.rt[1:3, 1:5]

# Two single cell data sets of Arabdopsis root: sc9, sc51.
sc.arab.rt9 <- scell_dat(paste0(dat.in, '/GSM4625994_sc_9_at_COPILOT.rds'), df.match=df.match.arab) # Read data.
as.matrix(sc.arab.rt9[1:3, 1:5])

sc.arab.rt51 <- scell_dat(paste0(dat.in, '/GSM4626006_sc_51_COPILOT.rds'), df.match=df.match.arab) # Read data.
